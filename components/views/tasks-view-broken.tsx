  ...
  return (
    <div className="p-4 md:p-8 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-xl md:text-2xl font-bold tracking-tight">EJECUCION DIARIA</h1>
          <p className="text-sm text-muted-foreground mt-1">
            {tasks.length}/5 TAREAS DEFINIDAS | {completedTasks.length} COMPLETADAS
          </p>
        </div>
      ...
      {/* Controles de Modo */}
      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-6 p-4 border border-border bg-card">
        <div className="flex items-center gap-3 min-w-0 flex-1">
          <Lock className={cn('h-4 w-4 flex-shrink-0', isLockdownMode && 'text-destructive')} />
          <div className="min-w-0 flex-1">
            <p className="text-xs font-bold tracking-wider">MODO BLOQUEO</p>
            <p className="text-[10px] text-muted-foreground">Bloquea navegacion hasta completar</p>
          </div>
          <Switch
            checked={isLockdownMode}
        ...
        <div className="flex items-center gap-3 min-w-0 flex-1">
          <Shield className={cn('h-4 w-4 flex-shrink-0', isForceMode && 'text-destructive')} />
          <div className="min-w-0 flex-1">
            <p className="text-xs font-bold tracking-wider">MODO FORZADO</p>
            <p className="text-[10px] text-muted-foreground">Restriccion maxima</p>
          </div>
          <Switch
            checked={isForceMode}
      ...
      {/* Advertencia si no hay tareas */}
      {tasks.length === 0 && (
        <div className="border border-warning/30 bg-warning/5 p-4 sm:p-6">
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 sm:gap-4">
            <AlertTriangle className="h-6 w-6 sm:h-8 sm:w-8 text-warning flex-shrink-0" />
            <div>
              <h3 className="font-bold text-base sm:text-lg">NO HAY TAREAS DEFINIDAS</h3>
              <p className="text-sm text-muted-foreground">
                No puedes ejecutar sin un plan. Define 3-5 tareas criticas AHORA.
              </p>
            </div>
          </div>
        </div>
      ...
      {/* Advertencia si hay pocas tareas */}
      {tasks.length > 0 && tasks.length < 3 && (
        <div className="border border-warning/30 bg-warning/5 p-3 sm:p-4">
          <div className="flex items-start gap-3">
            <AlertTriangle className="h-4 w-4 sm:h-5 sm:w-5 text-warning flex-shrink-0 mt-0.5" />
            <p className="text-sm text-muted-foreground">
              <span className="font-bold text-foreground">POCAS TAREAS.</span> Se recomiendan minimo 3 tareas para una ejecucion significativa.
            </p>
          </div>
        </div>
      )}
      ...
      {pendingTasks.length > 0 && (
        <div className="space-y-3">
          <h2 className="text-xs font-bold tracking-wider text-muted-foreground">
            PENDIENTES DE EJECUCION ({pendingTasks.length})
          </h2>
          <div className="space-y-2">
      ...
      {/* Mensaje de Todo Completado */}
      {allCompleted && (
        <div className="border border-success/30 bg-success/5 p-6 text-center">
          <h3 className="font-bold text-lg text-success">EJECUCION COMPLETA</h3>
          <p className="text-sm text-muted-foreground mt-1">
            Todas las tareas completadas. Te has ganado tu descanso.
          </p>
        </div>
      )}
    </div>
